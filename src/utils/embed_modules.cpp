...
namespace trnist::utils
{
	...
	{
		py::class_<translators::QtJsRuntime>(m, "Runtime")
			.def(py::init<>())
			.def("evaluate", &translators::QtJsRuntime::evaluate)
			.def("is_available", &translators::QtJsRuntime::is_available)
			.def_property_readonly("name", &translators::QtJsRuntime::name);

		py::class_<translators::QtJsCompileContext>(m, "RuntimeCompileContext")
			.def("execute", &translators::QtJsCompileContext::execute)
			.def("evaluate", &translators::QtJsCompileContext::evaluate)
			.def("call", &translators::QtJsCompileContext::call);

		py::class_<translators::TseWrapper>(m, "Tse")
			.def(py::init<>())
			.def("compile", &translators::TseWrapper::compile)
			.def("execute", &translators::TseWrapper::execute)
			.def("evaluate", &translators::TseWrapper::evaluate)
			.def_property_readonly("current_runtime", &translators::TseWrapper::current_runtime);

		m.attr("ExejsProgramError") = py::exception<translators::ExejsProgramError>(m, "ExejsProgramError");
		m.attr("ExejsProcessExitError") = py::exception<translators::ExejsProcessExitError>(m, "ExejsProcessExitError");
		m.attr("ExejsRuntimeUnavailableError") = py::exception<translators::ExejsRuntimeUnavailableError>(m, "ExejsRuntimeUnavailableError");

		m.attr("tse") = py::cast(translators::TseWrapper());
		m.attr("compile") = py::cpp_function([](const std::string &source, const std::string &cwd)
											 { return translators::TseWrapper().compile(source, cwd); });
		m.attr("execute") = py::cpp_function([](const std::string &source)
											 { return translators::TseWrapper().execute(source); });
		m.attr("evaluate") = py::cpp_function([](const std::string &source)
											  { return translators::TseWrapper().evaluate(source); });
		m.attr("runtime") = py::cast(translators::QtJsRuntime());
	}
